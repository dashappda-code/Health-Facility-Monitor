...
def create_filters(df):
    ...
    with refresh_col:
        if st.button("🔄 Refresh", key="refresh_data_button",
...
def apply_filters(df, selected_years, selected_months, selected_weeks,
                  selected_diseases, selected_wards, selected_genders,
                  selected_opd_ipd, date_from, date_to):
    out = df.copy()
    ...
    if date_from is not None and date_to is not None and "Reporting Date" in out.columns:
        out = out[
...
def calculate_kpis(df):
    disease = (df["Confirmed Diagnosis"].dropna().value_counts()
               if "Confirmed Diagnosis" in df.columns else pd.Series(dtype="int64"))
    ward = (df["Ward"].dropna().value_counts()
            if "Ward" in df.columns else pd.Series(dtype="int64"))
    facility = (df["Facility Name Lform"].dropna().value_counts()
                if "Facility Name Lform" in df.columns else pd.Series(dtype="int64"))

    return {
        "total": len(df),
        "opd": int(df["Opd Ipd"].eq("OPD").sum()) if "Opd Ipd" in df.columns else 0,
        "ipd": int(df["Opd Ipd"].eq("IPD").sum()) if "Opd Ipd" in df.columns else 0,
        "male": int(df["Gender"].eq("M").sum()) if "Gender" in df.columns else 0,
        "female": int(df["Gender"].eq("F").sum()) if "Gender" in df.columns else 0,
        "transgender": int(df["Gender"].eq("Transgender").sum()) if "Gender" in df.columns else 0,
        "top_disease": disease.index[0] if len(disease) else "N/A",
        "top_ward": ward.index[0] if len(ward) else "N/A",
        "top_facility": facility.index[0] if len(facility) else "N/A",
    }
...
def render_overview(df):
    ...
    st.caption("Public Health Surveillance and Management Dashboard")
