...
) if not map_summary.empty else 0
...
# Multiple pydeck maps inside tabs can remain mounted
# in the browser and cause:
...
if active_view == "Hotspots":
    ...
    if (
        hotspots is None
        or not isinstance(hotspots, pd.DataFrame)
        or hotspots.empty
    ):
        st.info(
...
# ========================================================
    ...
    st.caption(
        "High burden areas are shown with stronger/darker "
    ...
    if not isinstance(
        ...
        # ------------------------------------------------
        # ONE ACTIVE DISEASE MAP
        #
        # This is intentionally one map at a time.
        # Rendering every disease as a separate pydeck
        # canvas causes WebGL context exhaustion.
        # ------------------------------------------------
